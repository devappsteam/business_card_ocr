...
def save_upload_image(fileObj):
    """
    Salva a imagem enviada no objeto de arquivo (fileObj) e retorna o caminho do arquivo salvo.
    """
...
def array_to_json_format(numpy_array):
    """
    Converte um array NumPy para um formato JSON de lista de pontos (x, y).
    """
    points = []
...
class DocumentScan():
    ...
    def resizer(image, width=500):
        """
        Redimensiona a imagem para uma largura específica, mantendo a proporção.
        """
        h, w, c = image.shape
    ...
    def apply_brightness_contrast(input_img, brightness=0, contrast=0):
        """
        Aplica ajustes de brilho e contraste à imagem.
        """
        if brightness != 0:
            if brightness > 0:
                shadow = brightness
    ...
    def document_scanner(self, image_path):
        """
        Realiza a digitalização do documento, encontra os pontos do contorno e retorna esses pontos juntamente com o tamanho da imagem.
        """
    ...
    def calibrate_to_original_size(self, four_points):
        """
        Calibra a imagem escaneada para o tamanho original usando os quatro pontos encontrados.
        """
